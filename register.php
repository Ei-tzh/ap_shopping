...
if($_POST){
    if(empty($_POST['name']) || empty($_POST['email']) || empty($_POST['password']) || strlen($_POST['name'])<4 || empty($_POST['address']) || empty($_POST['phone']) || (strlen($_POST['phone'])<9 || strlen($_POST['phone'])>14 ) ){
        ...
        if($user){
            ...
            $userstmt=$db->prepare("INSERT INTO users(name,email,password,address,phone) VALUES (:name,:email,:password,:address,:phone)");
			...
			<div class="row">
                ...
                <div class="login_form_inner">
						...
						<input name="_token" type="hidden"  value="<?php echo $_SESSION['_token']; ?>">
							...
							<div class="col-md-12 form-group">
								<input type="text" class="form-control <?php echo empty($emailError) ? '': 'is-invalid border-danger'; ?>" id="email" name="email" placeholder="Email" onfocus="this.placeholder = ''" onblur="this.placeholder = 'email'">
                                <div class="invalid-feedback text-left">
                                    <?php echo empty($emailError) ? '': $emailError; ?>
                                </div>
							</div>
							<div class="col-md-12 form-group">
								<input type="password" class="form-control <?php echo empty($passwordError) ? '': 'is-invalid border-danger'; ?>" id="password" name="password" placeholder="Password" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Password'">
                                <div class="invalid-feedback text-left">
                                    <?php echo empty($passwordError) ? '': $passwordError; ?>
                                </div>
							</div>
							<div class="col-md-12 form-group">
                                <input type="text" class="form-control <?php echo empty($addressError) ? '': 'is-invalid border-danger'; ?>" id="address" name="address" placeholder="Address" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Address'">
                                <div class="invalid-feedback text-left">
                                    <?php echo empty($addressError) ? '': $addressError; ?>
                                </div>
							</div>
							<div class="col-md-12 form-group">
								<input type="tel" class="form-control <?php echo empty($telError) ? '': 'is-invalid border-danger'; ?>" id="phone" name="phone" placeholder="Phone.no" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Phone.no'">
                                <div class="invalid-feedback text-left">
